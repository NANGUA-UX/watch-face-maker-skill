...
复用现有采集器与Python校验器，不新增服务。采集器v2在异步读取前后分别核对内容及母件引用，覆盖文字分段样式、矢量路径、图片哈希、导出设置与布局。`stability.before/after` 必须等于 `capture_signature`。该签名用于发现变化，不等同于Figma服务器修订号，也不是视觉验收。
